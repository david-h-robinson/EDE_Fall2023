...
create_df <- data.frame("speed"=car_speed,"dist"=car_dist) #create new data frame rather than converting a vector into a data frame as as.data.frame does
create_df
```
...
summary(df)
head(df,2) #show the first 6 rows of df #second optional command specifies number of rows 

#If you know the number of the column you want you can refer to that to access column
df[1:10,2] #This is a slice -- column 2 for rows 1-10
df[c(1,2,3),2] #This can pluck items out by order within the data frame
df[c(2,1,3),] #This acceses row 2, row 1, and row 3 in that order. And then not specifying anything after, so it's give me all the columns.
df [c(2,1,3),]$car_speed #This returns car speed for the rows 2, 1, and 3 in that order
