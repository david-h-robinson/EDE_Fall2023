...
```{r project.setup, message=FALSE, warning=FALSE, result='hide'}
#Import libraries
...
```{r plot2018, echo=FALSE, fig.cap = "Ozone concentrations by month in 2018", fig.align = 'left', fig.height = 3.5}

epa_data_2018 <- epa_data %>%
  filter(Year == "2018")

Ozone_2018_boxplot <-
  ggplot(epa_data_2018,
         aes(x= factor(Month,
                          levels = 1:12,
                          labels = month.abb),
                y=Ozone))+
  geom_boxplot()+
  labs(x="", y= expression(paste("Ozone Concentration ","(",mu,"g / ",m^{3},")")))
...
```{r plot2019, echo=FALSE, fig.cap = "Ozone concentrations by month in 2019", fig.align = 'left', fig.height = 3.5}

epa_data_2019 <- epa_data %>%
  filter(Year == "2019")

Ozone_2019_boxplot <-
  ggplot(epa_data_2019,
         aes(x= factor(Month,
                          levels = 1:12,
                          labels = month.abb),
                y=Ozone))+
