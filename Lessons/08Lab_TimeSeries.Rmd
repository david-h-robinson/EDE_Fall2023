...
Rainfall_BR_processed <-
  ...
  separate(Statistics,c("Null","Month","Null2")," ") %>% #this works like text to columns in Excel #there's a sneaky space here before the month
  mutate( Date = my(paste0(Month,"-",Year))) %>% #this is "month-year" function from lubridate
...
#no NAs
#the ideal way to do this is to create a sequence from the first date to the start date, then left join this set with the date column of the dataset of interest -- if they all match, that means there is no missing data -- if they do not all match, there will be NAs that indicate missing data
...
ggplot(BR_complete, aes(x = Date,
                                  y = Rainfall)) +
  geom_line() +
  geom_smooth() #is "(method = lm)" needed here?
...
BR_complete_Rainfall_ts <- ts(BR_complete$Rainfall, start = c(1901,1), frequency = 12) #c(1901,1) drives the axis and the step

f_month = month(first(BR_complete$Date))
f_year = year(first(BR_complete$Date))

BR_complete_Temperature_C_ts <- ts(BR_complete$Temperature_C, start = c(f_year,f_month), frequency = 12) #c(1901,1) drives the axis and the step
...
#the bigger the grey bar on the right hand side, the smaller the impact it has on the data set

BR_complete_Temperature_C_Decomposed <- stl(BR_complete_Temperature_C_ts, s.window = "periodic")

plot(BR_complete_Temperature_C_Decomposed)
...
BR_complete_Rainfall_Components <- 
  as.data.frame(BR_complete_Rainfall_Decomposed$time.series[,1:3])

BR_complete_Rainfall_Components <-
  mutate(BR_complete_Rainfall_Components,
         Observed =
           )

BR_complete_Temperature_C_Components <- 
  as.data.frame(BR_complete_Temperature_C_Decomposed$time.series[,1:3])
