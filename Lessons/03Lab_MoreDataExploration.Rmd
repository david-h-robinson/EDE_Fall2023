...
# Matrices ----
matrix1 <- matrix(1:20, nrow = 5,ncol = 4) #this is default to fill by column
matrix1
matrix2 <- matrix(1:20, nrow = 5, ncol = 4, byrow = TRUE) #this fills by row
matrix2
...
matrix1[4, ] #this will return row 4, all elements
matrix1[ , 3] #
matrix1[c(12, 14)] #
matrix1[c(12:14)] #
matrix1[2:4, 1:3] #
...
#data frames and matrices show up as data
#variables and vectors show up as values
#functions show up as functions
...
```{r DataExercise-1a}
tenDayForecastHigh <- c(80,81,82,81,78,72,77,80,80,80)
tenDayForecastLow <- c(56,60,60,61,61,61,62,64,60,60)
tenDayForecastHigh
tenDayForecastLow
...
convert.F.to.C <- function(x){
  convertedTemps <- (x-32)*5/9
  print(convertedTemps)

  }

convert.F.to.C(tenDayForecastHigh)
convert.F.to.C(tenDayForecastLow)

#I did it like the above, Luana did as below -- didn't need a function

high_C <- (tenDayForecastHigh - 32) * 5/9
low_C <- (tenDayForecastLow - 32) * 5/9
...
df_temp <- data.frame(tenDayForecastHigh,tenDayForecastLow,high_C,low_C)
...
```{r DataExercise-1d}
summary(df_temp)
#sd(df_temp) #this will not work on the given data type
sd(df_temp$tenDayForecastHigh)
...
#Since the format is month-day-year we will use function mdy()
#date_juneteenth <- ()
date_obj_juneteenth <- dmy(str_juneteenth)
date_obj_juneteenth

#it will always print in the year, month, date format by default
...
str_past <- "55-feb-3"
#Alternative 1
date_obj_past <- fast_strptime(str_past,"%y-%b-%d",cutoff_2000=23L) #this would change to 24L once we hit 2024
date_obj_past
