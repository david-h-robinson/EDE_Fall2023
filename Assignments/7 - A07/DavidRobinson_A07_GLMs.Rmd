...
#QUESTION -- how to make this format prettier with lubridate?
...
> Answer: 
Temperature decreases as depth increases. The distribution suggests a Weak, negative correlation.
...
> Answer:
Based on the slope, we see that the temperature is decreasing by 1.94621 degrees per meter. 
Based on the R-squared value of 0.7387, around 73% of the variability in temperature is explained by changes in depth.
...
Based on the R-squared value of 0.7412, around 74% of the observed variance in this model is explained. Yes, this is a slight improvement over using only depth as the explanatory variable which had an R-squared value of 0.7387.
...
#QUESTION -- verify output
#QUESTION -- clean up graph to make it pretty
...
>Answer: 
Because the p-value is greater than 0.05, we cannot reject the null hypothesis (H0 = the mean temperature is the same between Crampton Lake and Ward Lake). Thus, the mean temperatures between the two lakes are the same.
